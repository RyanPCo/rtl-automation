...
(
    ...
    //--|Shift Register|----------------------------------------------------------
    always @(posedge clk) begin
        if (reset) rx_shift <= 10'b11_1111_1111;
    ...
    always @ (posedge clk) begin
        ...
        end else if (bit_counter == 1 && symbol_edge) begin
            uart_rx_overflow <= has_byte;
            has_byte <= 1'b1;
        end else begin
            uart_rx_overflow <= 1'b0;
            if (data_out_ready) has_byte <= 1'b0;
        end
    end
endmodule
